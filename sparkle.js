...
global.checkDjs = function() {
	...
	console.log('There are ' + (djs.length + extra) + ' djs');
	console.log(djs);
	console.log('min is ' + config.djing.minDjs + ' djs');
	console.log('max is ' + config.djing.maxDjs + ' djs');
	console.log('Does min jive? ' + (djs.length + extra >= config.djing.minDjs));
	console.log('Does max jive? ' + (djs.length + extra <= config.djing.maxDjs));
	if((djs.length + extra >= config.djing.minDjs && djs.length + extra <= config.djing.maxDjs) || djs.length < 1) {
		if(isdjing) {
			return;
		}
		bot.addDj();
